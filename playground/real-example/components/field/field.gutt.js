...
8: async function (layer) {
			...
			layer.lookahead[9] = layer.lookahead[8]
var initialScope = {"autocomplete": scope['autocomplete'],"type": scope['type'],"value": scope['value'],"disabled": scope['disabled'],"name": scope['name']}

		if (typeof layer.components[8] === 'undefined') {
			var result =await  imports['x-input'](layer.anchors[8].parentNode, layer.anchors[8], initialScope, state, layer.lookahead[8][0], anchor) 
			layer.components[8] = result.setState
			layer.elements[8] = result.elements
		} else {
