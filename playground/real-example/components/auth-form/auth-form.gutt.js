const main = async function (mountNode) {
	...
	var rootElements = []
	var imports = {
'x-field': import("../field/field.gutt.js"),
'x-button': import("../button/button.gutt.js")
	}
	var dynamicNodes = {
1: EXECUTE,
2: COMPONENT,
...
var initialScope = {"label": "Эл. почта","type": "email","autocomplete": "username","name": "email"}

		if (typeof layer.components[2] === 'undefined') {
			var result =await  imports['x-field'](layer.anchors[2].parentNode, layer.anchors[2], initialScope, state, layer.lookahead[2][0], anchor) 
...
4: async function (layer) {
			...
			layer.lookahead[5] = layer.lookahead[4]
var initialScope = {"label": "Пароль","type": "password","autocomplete": "current-password","name": "password"}

		if (typeof layer.components[4] === 'undefined') {
			var result =await  imports['x-field'](layer.anchors[4].parentNode, layer.anchors[4], initialScope, state, layer.lookahead[4][0], anchor) 
			layer.components[4] = result.setState
			layer.elements[4] = result.elements
		} else {
...
6: async function (layer) {
			...
			layer.lookahead[7] = layer.lookahead[6]
var initialScope = {"type": "submit","disabled": !scope['auth']['email'].length || !scope['auth']['password'].length}

		if (typeof layer.components[6] === 'undefined') {
			var result =await  imports['x-button'](layer.anchors[6].parentNode, layer.anchors[6], initialScope, state, layer.lookahead[6][0], anchor) 
			layer.components[6] = result.setState
			layer.elements[6] = result.elements
		} else {
