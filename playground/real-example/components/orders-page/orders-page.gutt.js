const main = async function (mountNode) {
	...
	var rootElements = []
	var imports = {
'fast-search': import("../fast-search/fast-search.gutt.js"),
'x-pagination': import("../pagination/pagination.gutt.js")
	}
	var dynamicNodes = {
1: EXECUTE,
2: COMPONENT,
...
var initialScope = {"placeholder": "Поиск по имени, фамилии, почте или дате"}

		if (typeof layer.components[2] === 'undefined') {
			var result =await  imports['fast-search'](layer.anchors[2].parentNode, layer.anchors[2], initialScope, state, layer.lookahead[2][0], anchor) 
...
forEach(scope['orders']['items'], async function (item, field) {
scope['order'] = item
await iteration(field, item)
})
})
...
14: async function (layer) {
			...
			layer.lookahead[15] = layer.lookahead[14]
var initialScope = {"length": scope['orders']['total'],"on-page": 100,"url": "/manager/","current-page": scope['page']}

		if (typeof layer.components[14] === 'undefined') {
			var result =await  imports['x-pagination'](layer.anchors[14].parentNode, layer.anchors[14], initialScope, state, layer.lookahead[14][0], anchor) 
			layer.components[14] = result.setState
			layer.elements[14] = result.elements
		} else {
